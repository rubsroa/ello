...
const labels = ["Prestation", "Date", "Heure", "Coordonnées", "Validation", "Confirmation"];
...
export function BookingWizard({
  ...
  const selectedPrice = service?.staff.find((link) => link.staff.id === selectedSlot?.staffId)?.customPrice ?? service?.priceCents;
  const groupedSlots = useMemo(() => Object.entries(Object.groupBy(slots, (slot) => new Date(slot.startsAt).toLocaleTimeString("fr-CH", { hour: "2-digit", minute: "2-digit" }))), [slots]);
  ...
  function chooseService(id: string) { setServiceId(id); setDate(""); setSlots([]); setStep(2); }
  function chooseAudience(nextAudience: "FEMALE" | "MALE") { setAudience(nextAudience); setServiceId(""); setDate(""); setSlots([]); }
  ...
  async function submitBooking() {
    ...
    try {
      ...
      setReference(body.reference); setStep(6);
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
      <div>
        ...
        <p className="eyebrow text-brass">Étape {step} sur 6</p>
        ...
        {step === 2 && <div className="mt-8"><p className="mb-6 max-w-xl text-sm font-light leading-6 text-night/60">Votre rendez-vous est pris directement avec le salon. Choisissez simplement le jour qui vous convient.</p><label className="eyebrow block text-night/60" htmlFor="booking-date">Choisir une date</label><input id="booking-date" type="date" min={format(new Date(), "yyyy-MM-dd")} max={format(addDays(new Date(), maximumAdvanceDays), "yyyy-MM-dd")} value={date} onChange={(event) => { setDate(event.target.value); void loadSlots(event.target.value); setStep(3); }} className="mt-3 min-h-14 w-full border border-night/20 bg-white px-4 text-lg sm:max-w-sm" /></div>}

        {step === 3 && <div className="mt-8">{loadingSlots ? <LoaderCircle className="animate-spin" aria-label="Chargement des créneaux" /> : groupedSlots.length ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{groupedSlots.map(([time, matching]) => { const slot = matching?.[0]; if (!slot) return null; return <button type="button" key={time} onClick={() => { setStartsAt(slot.startsAt); setStep(4); }} className="border border-night/20 bg-white px-4 py-4 font-normal transition hover:border-night hover:bg-night hover:text-ivory">{time}</button>; })}</div> : <p className="text-night/65">Aucun créneau disponible ce jour. Essayez une autre date.</p>}</div>}

        {step === 4 && <form className="mt-8 grid gap-5 sm:grid-cols-2" onSubmit={(event) => { event.preventDefault(); setStep(5); }}>{customerFields.map((field) => <label key={field.name} className="text-sm font-normal">{field.label}<input required name={field.name} type={field.type} autoComplete={field.autoComplete} value={form[field.name]} onChange={(event) => setForm({ ...form, [field.name]: event.target.value })} className="mt-2 min-h-13 w-full border border-night/20 bg-white px-4 font-light" /></label>)}<label className="text-sm font-normal sm:col-span-2">Message facultatif<textarea rows={3} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="mt-2 w-full border border-night/20 bg-white p-4 font-light" /></label><label className="flex items-start gap-3 text-sm font-light sm:col-span-2"><input type="checkbox" checked={form.marketingConsent} onChange={(event) => setForm({ ...form, marketingConsent: event.target.checked })} className="mt-1" />Je souhaite recevoir occasionnellement les nouvelles d’ell’o. Ce consentement est facultatif.</label><button className="button bg-night text-ivory sm:col-span-2" type="submit">Vérifier mon rendez-vous</button></form>}

        {step === 5 && <div className="mt-8"><p className="text-night/65">Vérifiez les détails ci-contre. Le créneau sera contrôlé une dernière fois au moment de la confirmation.</p><button type="button" disabled={submitting} onClick={() => void submitBooking()} className="button mt-8 bg-night text-ivory disabled:opacity-50">{submitting ? <LoaderCircle className="animate-spin" size={17} /> : <Check size={17} />} Confirmer le rendez-vous</button></div>}
